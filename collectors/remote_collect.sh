...
collect() {
	...
	trace $ssh_pre scp $SSH_OPTS $D/collect.sh $sshalias:$tmp_dir/netmap/
	trace $ssh_pre ssh -t $SSH_OPTS $sshalias "TCPDUMP_TIME=$TCPDUMP_TIME TCPDUMP_PACKETS=$TCPDUMP_PACKETS $tmp_dir/netmap/collect.sh $gendata_opts $tmp_dir/netmap/data $ip"
	if [ $k8s -eq 1 ]; then
		trace $ssh_pre scp $SSH_OPTS $D/collect.sh $sshalias:$tmp_dir/netmap/
		trace $ssh_pre ssh -t $SSH_OPTS $sshalias "SSH_NODES_CMD="$SSH_NODES_CMD" TCPDUMP_TIME=$TCPDUMP_TIME TCPDUMP_PACKETS=$TCPDUMP_PACKETS $tmp_dir/netmap/collect.sh $gendata_opts $tmp_dir/netmap/data $ip"
	fi
...
echo "$now $(whoami)@$(hostname) remote $* SSH_OPTS=$SSH_OPTS" >> "$output_dir/remote_collect_${now}.log"
